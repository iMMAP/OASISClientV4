...
152         If i = 1 Then
154             displayProgressBar 0, 250, lProgressBarWidths(i), fraProgressBars.Height - 250, fraProgressBars, lColours(0), lColours(i)
156             displayRatioTxt 0, 0, lProgressBarWidths(i), 250, fraProgressBars, lRatios(i)
            Else
158             displayProgressBar dxProgressBar(i - 1).Left + lProgressBarWidths(i - 1) - 5, 250, lProgressBarWidths(i), fraProgressBars.Height - 250, fraProgressBars, lColours(i - 1), lColours(i)
160             displayRatioTxt dxProgressBar(i - 1).Left + lProgressBarWidths(i - 1) - 5, 0, lProgressBarWidths(i), 250, fraProgressBars, lRatios(i)
            End If
...
Private Sub displayProgressBar(X As Integer, _
                               ...
                               w As Long, _
                               H As Integer, _
                               ByVal cContainingFrame As Frame, _
...
102     Set dxProgressBar(dxProgressBar.UBound).Container = cContainingFrame
...
102     Set txtRatio(txtRatio.UBound).Container = cContainingFrame
...
Private Sub UserControl_Initialize()
        '100     dxStartColour = vbWhite
        '102     'Call dxThemeIntervals_Change
        '<EhHeader>
...
UserControl_Initialize_Err:
        Err.Raise vbObjectError + 100, _
                  "OASISThemeRange.OASISThemeRangePicker.UserControl_Initialize", _
                  "OASISThemeRangePicker component failure"
        '</EhFooter>
End Sub
