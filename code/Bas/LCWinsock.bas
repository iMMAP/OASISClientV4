...
Public Function SetSockLinger(ByVal SockNum&, ByVal OnOff%, ByVal LingerTime%) As Long
  ...
  Dim Linger As LingerType
    ...
    If setsockopt(SockNum, SOL_SOCKET, SO_LINGER, Linger, 4) Then
        DebugPrint "Error setting linger info: " & WSAGetLastError()
        SetSockLinger = SOCKET_ERROR
      Else
        If getsockopt(SockNum, SOL_SOCKET, SO_LINGER, Linger, 4) Then
            DebugPrint "Error getting linger info: " & WSAGetLastError()
            SetSockLinger = SOCKET_ERROR
          Else
            DebugPrint "Linger is on if nonzero: " & Linger.l_onoff
            DebugPrint "Linger time if linger is on: " & Linger.l_linger
        End If
    End If
  ...
  Dim StartupData As WSADataType
    ...
    If Not WSAStartedUp Then
        If Not WSAStartup(&H101, StartupData) Then
            WSAStartedUp = True
            DebugPrint "wVersion=" & StartupData.wVersion
            DebugPrint "wHighVersion=" & StartupData.wHighVersion
            DebugPrint "If wVersion = 257 then everything is OK"
            DebugPrint "szDescription=" & StartupData.szDescription
            DebugPrint "szSystemStatus=" & StartupData.szSystemStatus
            DebugPrint "iMaxSockets=" & StartupData.iMaxSockets
            DebugPrint "iMaxUdpDg=" & StartupData.iMaxUdpDg
