...
Begin VB.Form frmOASISProgress 
   ...
   ClientWidth     =   5955
   ControlBox      =   0   'False
   Icon            =   "frmOASISProgressAdmin.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   1440
   ...
   Begin OASISRemoteAdmin.MsgScroller MsgScroller1 
      Height          =   285
      Left            =   60
      Top             =   1110
      Width           =   5835
...
104     iSetTimeoutSeconds = sTimeout
106     iSetNumberOfRetries = sRetries

108     With ooConn
110         .CursorLocation = adUseClient
112         .ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;" & "Data Source=" & sClientDbFullPath
114         .open
        End With
        
116     With oRs
118         Set .ActiveConnection = ooConn
120         .CursorType = adOpenDynamic
122         .LockType = adLockBatchOptimistic
124         .Source = "SELECT * FROM AppSettings WHERE SettingName = 'ServerConnectionParameters'"
126         .CursorLocation = adUseClient
128         .open
        End With
        
130     If oRs.State = adStateOpen Then
    
132         If Not oRs.EOF And Not oRs.Bof Then
    
134             oRs.fields("SettingValue1").Value = sTimeout
136             oRs.fields("SettingValue2").Value = sRetries

            Else
138             oRs.AddNew
140             oRs.fields("SettingName").Value = "ServerConnectionParameters"
142             oRs.fields("SettingValue1").Value = sTimeout
144             oRs.fields("SettingValue2").Value = sRetries
            End If
        
        End If
    
146     oRs.UpdateBatch adAffectCurrent
148     oRs.Save
    
150     Set oRs = Nothing
152     Set ooConn = Nothing

154     InitialiseProgressForm sClientDbFullPath
...
SetClientDBTimeoutSettings_Err:
        'MsgBox Err.Description & vbCrLf & _
               "in OASISRemoteAdmin.frmOASISProgress.SetClientDBTimeoutSettings " & _
               "at line " & Erl
        'Resume Next
        '</EhFooter>
End Sub
...
144             iSetTimeoutSeconds = 30
146             iSetNumberOfRetries = 3
            End If
        
        Else
    
148         iSetTimeoutSeconds = 30
...
156     m_frmDebug.DebugPrint "Server communication timeout = " & iSetTimeoutSeconds
158     m_frmDebug.DebugPrint "Server communication retries = " & iSetNumberOfRetries
...
InitialiseProgressForm_Err:
        ...
        iSetTimeoutSeconds = 30
        iSetNumberOfRetries = 3
        'Resume Next
        '</EhFooter>
End Sub
...
Private Sub Timer1_Timer()
    ...
    m_frmDebug.DebugPrint lCountOfSeconds
...
Private Function URLEncode(strWhat As String) As String
...
Public Function OpenHttpCommsRS(sSQL As String, _
        ...
        Dim sWebsiteLocal As String
        Dim sFunction As String
        Dim sParameter As String
        
        
        
        sWebsiteLocal = Left$(sSQL, InStr(LCase$(sSQL), "oasis.asp") + 8)

        If Not InStr(LCase$(sSQL), "=") > 0 Then
            sFunction = Right$(sSQL, Len(sSQL) - Len(sWebsiteLocal) - 1)
            sParameter = ""
        Else
            sFunction = Mid$(sSQL, InStr(LCase$(sSQL), "oasis.asp") + 10, InStr(LCase$(sSQL), "=") - InStr(LCase$(sSQL), "?") - 1)
            sParameter = Right$(sSQL, Len(sSQL) - Len(sWebsiteLocal) - Len(sFunction) - 2)
        End If
        
        If Right(sWebsiteLocal, 11) = "//oasis.asp" Then
            sWebsiteLocal = Left(sWebsiteLocal, Len(sWebsiteLocal) - 11) & "/oasis.asp"
        End If
        
        If Not bSilent Then
            Me.Show vbModeless
            'InitProgressBar
            Label1.Caption = "Receiving from: " & sWebsiteLocal
            MsgScroller1.Clear
            MsgScroller1.AddItem "Fetching data from server...", "test", 0
            Me.Refresh
        End If

        m_frmDebug.DebugPrint "sWebsiteLocal: " & sWebsiteLocal
        m_frmDebug.DebugPrint "sFunction: " & sFunction
        m_frmDebug.DebugPrint "sParameter: " & sParameter
...
100     sFunction = UCase(sFunction)
        sURLEncoded = URLEncode(sFunction & "=" & CheckEncrypt(sParameter))

        MsXmlHttp.open "POST", sWebsiteLocal, False
        MsXmlHttp.setRequestHeader "Content-type", "application/x-www-form-urlencoded"
        MsXmlHttp.setRequestHeader "Content-length", Len(sURLEncoded)
        
        If frmDatabaseConnect.g_bProxyEnabled Then MsXmlHttp.setProxy 2, frmDatabaseConnect.g_sProxy
        MsXmlHttp.send sURLEncoded

114     If Not Trim(MsXmlHttp.responseText) = "-1" And Not Trim(MsXmlHttp.responseText) = "" Then

116         sXMLString = MsXmlHttp.responseText
            Set OpenHttpCommsRS = RecordsetFromXMLString(sXMLString)

        Else
136         Set OpenHttpCommsRS = Nothing
        End If
        
138     MsXmlHttp.abort
140     Set MsXmlHttp = Nothing
...
OpenHttpCommsRS_Err:
        MsgBox "OpenHttpCommsRS_Err: (" & Erl & ") " & Err.Description
        Set OpenHttpCommsRS = Nothing
        Me.Hide
        ...
        '        '</EhHeader>
        '
        '        Dim MsXmlHttp As MSXML2.ServerXMLHTTP40
        '100     Set MsXmlHttp = New MSXML2.ServerXMLHTTP40
        '
        '102     If bInit Then
        '
        ...
        '116         m_frmDebug.DebugPrint Label1.Caption
        '
        '            On Error Resume Next
        '
        ...
        '        End If
        '
        '        m_frmDebug.DebugPrint "sql: " & sSQL
        ...
        '
        '            On Error GoTo OpenHttpCommsRS_Err
        '140         m_frmDebug.DebugPrint "succeeded on attempt: " & CStr(iAttemptCount)
        '
        ...
        '                ' Response Desirable
        '
        '144             m_frmDebug.DebugPrint "Recordset returned"
        '
        ...
        '
        '148             m_frmDebug.DebugPrint "(" & MsXmlHttp.responseText & ") Recordset not received from server"
        '
        ...
        '154         m_frmDebug.DebugPrint "failed attempt: " & CStr(iAttemptCount - 1)
        '
        ...
        '
        'connectionfails:
        '        m_frmDebug.DebugPrint "Connection failed on attempt " & iAttemptCount & ". Err Message: " & Err.Description
        '        MsXmlHttp.abort
...
Public Function SaveHttpCommsRS(oRs As ADODB.Recordset, _
    ...
    sParam = Replace(LCase$(sWebsite), "oasis.asp", "oasisuploader.asp")

    If Not bSilent Then
        Me.Show vbModeless
        'InitProgressBar
        Label1.Caption = "Saving to: " & sParam
        MsgScroller1.Clear
        MsgScroller1.AddItem "Saving data to server...", "test", 0
        Me.Refresh
    End If

    MsXmlHttp.open "POST", sParam, 0
    oRs.Save MsXmlDoc, 1
    If frmDatabaseConnect.g_bProxyEnabled Then
        MsXmlHttp.setProxy 2, frmDatabaseConnect.g_sProxy
    End If
    MsXmlHttp.send MsXmlDoc

    If Not Trim(MsXmlHttp.responseText) = "Data Updated" Then
        MsgBox Trim(MsXmlHttp.responseText)
        SaveHttpCommsRS = False
    Else
        SaveHttpCommsRS = True
    End If
    ...
    '
    '122         m_frmDebug.DebugPrint "Saving RS with " & oRs.RecordCount & " record(s) to sWebsite: " & sWebsite
    '            On Error Resume Next
    '
    '            If Not bSilent Then
    '
    ...
    '                    m_frmDebug.DebugPrint Label1.Caption
    '                End If
    '
    ...
    '                    MsXmlHttp.abort
    '                    m_frmDebug.DebugPrint "Save to server not confirmed...."
    '                End If
    '
    '            Loop
    '
    '        Else
    '
    '160         m_frmDebug.DebugPrint "SaveHttpCommsRS: Recordset is either BOF or EOF - this may be a desired behaviour"
    '
...
114         Label1.Caption = "***** Attempt " & iAttemptCount & " of " & iSetNumberOfRetries & " *****"
116         m_frmDebug.DebugPrint Label1.Caption
118         m_frmDebug.DebugPrint "sql: " & sSQL
            On Error Resume Next
...
134     MsXmlHttp.send
        ...
        If MsXmlHttp.waitForResponse(iSetTimeoutSeconds) Then
            ...
            On Error GoTo OpenHttpCommsResponse_Err
138         m_frmDebug.DebugPrint "succeeded on attempt: " & CStr(iAttemptCount) & " returned: " & MsXmlHttp.responseText
...
146         m_frmDebug.DebugPrint "failed attempt: " & CStr(iAttemptCount - 1)
